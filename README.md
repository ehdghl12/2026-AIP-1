...
## 프로젝트의 플로우 차트
<p align="center">
  <img src="Souce/flowchart.png" width="950">
</p>
...
<table>
  ...
  <tr>
    <td>전동킥보드</td>
    <td><img src="Souce/Kickboard.png" width="250"></td>
    <td>BBox</td>
    <td>7,000장</td>
  ...
  <tr>
    <td>맨홀</td>
    <td><img src="Souce/Manhole.png" width="250"></td>
    <td>BBox</td>
    <td>1,000장</td>
    <td>위험 객체 거리 판정</td>
  </tr>
  ...
  <tr>
    <td>소화전</td>
    <td><img src="Souce/fire_hydrant.jpeg" width="250"></td>
    <td>BBox</td>
    <td>1,000장</td>
    <td>위험 객체 거리 판정</td>
  </tr>
  ...
  <tr>
    <td>점자블록</td>
    <td><img src="Souce/tactile_paving.jpeg" width="250"></td>
    <td>BBox</td>
    <td>1,000장</td>
  ...
  <tr>
    <td>횡단보도</td>
    <td><img src="Souce/crosswalk.jpeg" width="250"></td>
    <td>Polygon</td>
    <td>1,000장</td>
    <td>주차 금지 영역 판정</td>
  </tr>
...
## 모델 구성
본 프로젝트에서는 두 개의 YOLO11 모델을 사용합니다. 두 모델을 하나로 병합하지 않고, 앱 내부에서 객체 검출 모델(Detection)과 영역 분할(Segmentation) 모델이 각각 독립적으로 동작하도록 구성하였습니다.
...
두 모델의 역할을 분리한 이유는 객체별 라벨 형식과 판단 방식이 다르기 때문입니다. 맨홀, 점자블록, 소화전은 BBox 중심점 기반 거리 계산에 적합하고, 횡단보도는 하나의 영역으로 판단해야 하므로 Polygon 기반 세그멘테이션이 더 적합합니다.


## 주차 판정 로직
주차 판정은 킥보드와 위험 객체 사이의 정규화 거리(Normalized Distance)를 기준으로 수행합니다. 사진은 촬영 위치, 각도, 거리, 렌즈 시야각에 따라 실제 거리와 픽셀 거리가 다르게 나타냅니다. 따라서 실제 물리 거리를 직접 계산하기보다, 사진 속 킥보드 높이를 기준으로 상대 거리를 정규화하여 판단합니다.
<p align="center">
  <img src="Souce/parkinglogic.png" width="950">
</p>
...
## iOS기반 온비다이스 앱 구현
두 YOLO11 모델이 온디바이스 형태로 탑재되었고 온디바이스 성능(추론 속도 및 메모리 사용량) 추적과 Confidence Threshold를 확인하기 위한 기능도 포함되어 있습니다.

### 앱의 실제 동작 화면
<p align="center">
  <img src="Souce/inapp.png" width="950">
</p>

### 상세기능
| 기능 | 설명 |
| --- | --- |
| 객체 검출 | 사진 속 탐지 객체와 BBox 결과를 확인 |
...
## 필드 테스트
프로젝트 수행기간 동안의 필드 테스트는 총 3차례 진행하였습니다. 주간과 야간 환경을 모두 포함하여 객체 탐지 성능과 주차 판정 로직의 현장 적용 가능성을 확인하였습니다.

### 테스트 환경
| 차수 | 일자 | 장소 | 시간 |
| --- | --- | --- | --- |
| 1차 | 2026/06/03 | 캠퍼스 주변 | 주간 |
